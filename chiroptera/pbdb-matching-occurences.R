...
chiroptera_occ <- ggplot(chiroptera, aes( x = species, maxage, colour = genus))
...
diversity <-  chiroptera%>% gather(key = type, value = age, V3, V4) %>% mutate(count = ifelse(type == "maxage", 1, -1)) %>% group_by(age) %>% summarise(count = sum(count))  %>% arrange(-age, -count) %>% mutate(diversity = cumsum(count)) 
