...
## Methods
Initially, Chiroptera data was extracted from The Paleobiology Database (PBDB), which downloaded a comma-separated value (csv) fossil record with accompanying data on minimum age, maximum age, early interval of appearance, and late interval of appearance submitted by researchers. This PBDB data had varying levels of identification for fossils, with some only being defined at the suborder level and others at the species level. We parsed through this data through the Linux shell to specify fossils defined to the species level and shortened the data from the 1220 occurrences to 417. We then separated the column of accepted name into genus and species, and created a new csv file and included minimum age and maximum age in order to graph Chiroptera fossil occurrences through time. 
